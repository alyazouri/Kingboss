// ═══════════════════════════════════════════════════════════════
//  PUBG JORDAN ROUTING ENGINE v34.3 — "BLITZ RECRUITMENT" EDITION
//  - Pure Jordanian Lobbies & Matchmaking
//  - 10x Faster Crew Recruitment (Ultra-low latency)
//  - Allows non-JO friends to join your JO lobby seamlessly
// ═══════════════════════════════════════════════════════════════

var CONFIG = {
    VERSION:               "34.3-JO-BLITZ-RECRUIT",
    TARGET_PING:           4,
    SOCIAL_API_TARGET:     3,
    EXCELLENT_PING:        6,
    GOOD_PING:             10,
    ...
    CREW_MAX_PING:              25,       // أقصى تأخير مسموح للتجنيد
    CREW_CHAIN_LENGTH:          1,        // ⚡ نود واحد فقط للتجنيد = سرعة فائقة
...
// ═══════════════════════════════════════════════════════════════
// 🔥 REAL JORDAN OPTIMIZED PROXIES (Pure JO Nodes)
// ═══════════════════════════════════════════════════════════════

var PROXY_REGISTRY = {

    // ===== CORE FAST (For Crew & Recruit - 10x Speed) =====
    C01: { ip:"213.186.163.147", port:80,  carrier:"ORANGE", tier:0, targetPing:22, location:"JO", crewCore:true, burst:true, socialOk:true },
    C02: { ip:"213.186.163.24",  port:80,  carrier:"ZAIN",   tier:0, targetPing:24, location:"JO", crewCore:true, burst:true, socialOk:true },
    C03: { ip:"213.186.167.51",  port:80,  carrier:"ORANGE", tier:0, targetPing:24, location:"JO", crewCore:true, burst:true, socialOk:true },
    C04: { ip:"109.237.193.246", port:80,  carrier:"JO",     tier:0, targetPing:18, location:"JO", crewCore:true, burst:true, socialOk:true, ultraBurst:true },

    // ===== LOBBY (For Matchmaking & Finding JO Players) =====
    L01: { ip:"213.186.162.154", port:80,  carrier:"ZAIN",   tier:0, targetPing:26, location:"JO", lobbyOptimized:true, burst:true },
    L02: { ip:"213.186.161.26",  port:80,  carrier:"ORANGE", tier:0, targetPing:24, location:"JO", lobbyOptimized:true, burst:true },
    L03: { ip:"213.186.164.146", port:80,  carrier:"JO",     tier:0, targetPing:25, location:"JO", lobbyOptimized:true, burst:true },
    L04: { ip:"213.186.169.251", port:80,  carrier:"JO",     tier:0, targetPing:21, location:"JO", lobbyOptimized:true, burst:true, ultraBurst:true },

    // ===== SOCIAL / DISCOVERY (To show you searching for JO) =====
    S01: { ip:"213.186.160.242", port:80,  carrier:"JO",     tier:0, targetPing:25, location:"JO", socialOptimized:true, burst:true },
    S02: { ip:"213.186.161.187", port:80,  carrier:"JO",     tier:0, targetPing:29, location:"JO", socialOptimized:true, burst:true },
    S03: { ip:"213.186.174.174", port:80,  carrier:"JO",     tier:0, targetPing:24, location:"JO", socialOptimized:true, burst:true },

    // ===== OPEN PORT GAME UDP STYLE NODES =====
    U01: { ip:"109.237.197.184", port:20001, carrier:"JO",   tier:0, targetPing:28, location:"JO", crewCore:true, ultraBurst:true },
    U02: { ip:"109.237.205.83",  port:20003, carrier:"JO",   tier:0, targetPing:30, location:"JO", crewCore:true, ultraBurst:true },
    U03: { ip:"213.186.162.35",  port:20002, carrier:"JO",   tier:0, targetPing:32, location:"JO", crewCore:true, ultraBurst:true }
};
...
var POOL_LOBBY   = ["L04", "L02", "L01", "L03"]; // نبدأ بالأسرع
var POOL_SOCIAL  = ["S01", "S03", "S02"];
var POOL_CREW    = ["C04", "C01", "U01", "C02", "C03", "U02", "U03"]; // C04 الأسرع (18ms) دائماً بالبداية للتجنيد
var POOL_GENERAL = ["L04", "S01", "C04", "L02"];
...
var JO_NETWORKS = [
    ...
    // أضفنا الرينج الجديد 213.186.x.x ليكون معترفاً به كأردني بيور
];
...
function classifyTraffic(host) {
    // فحص التجنيد أولاً لضمان السرعة القصوى
...
function scoreProxy(proxy, modeKey) {
    ...
    if (modeKey === "CREW_FORCE") {
        score += (50 - proxy.targetPing) * 8; // مضاعفة مكافأة السرعة
