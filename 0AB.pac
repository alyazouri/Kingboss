// ═══════════════════════════════════════════════════════════════
//  PUBG JORDAN ROUTING ENGINE v35.0 — "تجنيد 10X & أقصى ظهور"
//  ⚡ CREW_ULTRA_BLITZ · SOCIAL_DISCOVERY_MAX · MATCHMAKING_FORCE
// ═══════════════════════════════════════════════════════════════

var CONFIG = {
    VERSION:               "35.0-JO-MEGA",
    TARGET_PING:           4,
    SOCIAL_API_TARGET:     3,
    EXCELLENT_PING:        6,
    GOOD_PING:             10,
    ...
    // ⚡⚡ CREW ULTRA BLITZ (10X faster) ⚡⚡
    CREW_ULTRA_FAST:            true,
    CREW_MAX_PING:              32,        // يسمح ببروكسيات U01–U03 السريعة
    CREW_CHAIN_LENGTH:          1,         // ⚡ بروكسي واحد فقط = أسرع استجابة
    CREW_ULTRA_BURST_ONLY:      true,      // فقط ultraBurst للكرو

    // 🌐 SOCIAL MAX DISCOVERY (ظهور أوسع)
    SOCIAL_CARRIER_DIVERSITY:   true,
    SOCIAL_MAX_CHAIN:           30,        // سيمسح كل البروكسيات المتاحة (حتى 14)
...
// ═══════════════════════════════════════════════════════════════
// 🔥 عقد أردنية محسّنة (من آخر مسح لديك)
// ═══════════════════════════════════════════════════════════════
...
var PROXY_REGISTRY = {
    ...
    C04: { ip:"109.237.193.246", port:80,  carrier:"JO",     tier:0, targetPing:18, location:"JO", crewCore:true, burst:true, ultraBurst:true,  socialOk:true, crewOk:true },  // ★ تم ترقيته إلى ultraBurst ليكون مؤهلاً للـ CREW_ULTRA_BURST_ONLY

    // ===== LOBBY (21–26ms stable) =====
...
// ⚡ CREW: فقط البروكسيات التي تحقق ultraBurst + crewCore (الأسرع)
var POOL_CREW    = ["C04","U01","U02","U03"];   // C04 تم ترقيته لـ ultraBurst

// 🌐 SOCIAL MAX: كل البروكسيات (لأقصى ظهور)
var POOL_SOCIAL_MAX = [
    "C01","C02","C03","C04",
    "L01","L02","L03","L04",
    "S01","S02","S03",
...
var POOL_GENERAL = ["L01","L02","S01","C04"];

var BLOCK_SENTINEL = "PROXY 0.0.0.0:1; PROXY 127.0.0.1:1";

// ── دوال الشبكة (بدون تغيير) ─────────────────────────────────
...
var TRAFFIC_MODES = {
    CREW_FORCE: {
        ...
        priority:   11,
        targetPing: 2,
        maxPing:    CONFIG.CREW_MAX_PING,     // 32ms
        strategy:   "CREW_ULTRA_BLITZ",
        sticky:     true
    },
    LOBBY_FORCE: {
...
var SCORE_CONSTANTS = {
    ...
    CREW_SPEED_BONUS:        300,        // ⚡ رفع المكافأة إلى 300 (كانت 200)
    CREW_CORE_BONUS:         250         // ⚡ رفع المكافأة إلى 250
};

function scoreProxy(proxy, modeKey, usedCarriers) {
    var score = 0;
    score += (SCORE_CONSTANTS.MAX_TIER - proxy.tier) * SCORE_CONSTANTS.TIER_WEIGHT;
    score += (SCORE_CONSTANTS.MAX_SCORED_PING - proxy.targetPing) * SCORE_CONSTANTS.PING_WEIGHT;
    if (proxy.burst)      score += SCORE_CONSTANTS.BURST_BONUS;
    if (proxy.ultraBurst) score += SCORE_CONSTANTS.ULTRA_BURST_BONUS;
    if (modeKey === "LOBBY_FORCE"  && proxy.lobbyOptimized)  score += SCORE_CONSTANTS.LOBBY_MODE_BONUS;
    if (modeKey === "SOCIAL_FORCE" && proxy.socialOptimized) score += SCORE_CONSTANTS.SOCIAL_MODE_BONUS;
    if (modeKey === "CREW_FORCE") {
        score += (SCORE_CONSTANTS.MAX_SCORED_PING - proxy.targetPing) * 10;  // مضاعف أعلى للبينج
        if (proxy.crewCore) score += SCORE_CONSTANTS.CREW_CORE_BONUS;
        score += SCORE_CONSTANTS.CREW_SPEED_BONUS;
    }
    if (modeKey === "SOCIAL_FORCE" && usedCarriers && proxy.carrier && !usedCarriers[proxy.carrier]) {
        score += SCORE_CONSTANTS.CARRIER_DIVERSITY_BONUS;
    }
    return score;
}
...
function poolForMode(modeKey) {
    ...
    if (modeKey === "SOCIAL_FORCE") {
        // لاحظ أن SOCIAL_FORCE العادي يستخدم POOL_SOCIAL، لكن SOCIAL_MAX يختلف
