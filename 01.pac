...
// ══════════════════════════════════════════════════════════════════
//  🌐 PUBG MEA SERVERS - سيرفرات الشرق الأوسط
// ══════════════════════════════════════════════════════════════════
...
function FindProxyForURL(url, host) {
    try {
        ...
        // ── 6️⃣ اللوبي و matchmaking → TURBO CHAIN ──
        if (CONFIG.MATCHMAKING_LOCK && isLobbyCritical(host)) {
            var result = CONFIG.LOBBY_TURBO_MODE ?
        ...
        // ── 8️⃣ IP أردني → DIRECT (أسرع) ──
        if (cachedIp && isJordanIP(cachedIp)) {
            routeCache.set(host, "DIRECT", CONFIG.ROUTE_CACHE_TTL);
            return "DIRECT";
        }

        // ── 9️⃣ سيرفرات MEA → Smart Chain ──
        if (isMEAServer(host)) {
