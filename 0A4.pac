// ═══════════════════════════════════════════════════════════════════════════
//  PUBG JORDAN ROUTING ENGINE  v34.2 — "تجنيد فائق السرعة" EDITION
//
//  ⚡ المشكلة: الكرو ما بيجو عالفريق بسرعة → زمن الاستجابة للتجنيد عالي
//  ⚡ السبب:   طلبات الكرو كانت مختلطة مع طلبات السوشيال العامة
//              وتستخدم سلسلة طويلة (10 بروكسيات) مع تنوع شركات بطيء
//  ⚡ الحل:    مود CREW_FORCE مخصص بمسلك فائق السرعة —
//              فقط أسرع 3-4 بروكسيات بدون تنوع، بدون تأخير.
//
//  الفرق الجوهري عن v34.1:
//    • CREW_FORCE  — مود جديد منفصل عن SOCIAL_FORCE
//    • أسرع بروكسيات فقط: C01 (4ms) · C02 (5ms) · C03 (5ms) · S01 (24ms)
//    • سلسلة قصيرة: 3 بروكسيات فقط → فشل سريع → التالي مباشرة
//    • بدون carrier diversity — السرعة أهم من التنوع للتجنيد
//    • ماكس ping: 30ms فقط (يمنع دخول البروكسيات البطيئة)
//    • SOCIAL_FORCE يبقى للبحث والاكتشاف (التنوع مهم هناك)
// ═══════════════════════════════════════════════════════════════════════════
...
var CONFIG = {
    ...
    // ── 🔥 SOCIAL DISCOVERY MAX (للبحث والاكتشاف فقط) ──────────────────
    JORDAN_DISCOVERY_MAX:       true,
    SOCIAL_CARRIER_DIVERSITY:   true,
    SOCIAL_MAX_CHAIN:           10,
    SOCIAL_BURST_REQUIRED:      false,

    // ── ⚡ CREW BLITZ MODE (للتجنيد السريع) ─────────────────────────────
    CREW_ULTRA_FAST:            true,   // ⭐ تفعيل مسلك الكرو فائق السرعة
    CREW_MAX_PING:              30,     // أقصى ping مسموح لكرو (يمنع البطيء)
    CREW_CHAIN_LENGTH:          3,      // ⚡ 3 بروكسيات فقط — فشل سريع
    CREW_ULTRA_BURST_ONLY:      true,   // فقط بروكسيات ultraBurst للكرو

    // ── Scoring Weights ─────────────────────────────────────────────────
    SOCIAL_PRIORITY_MULTIPLIER: 8.0,
    VISIBILITY_BOOST:           15.0,
    CARRIER_DIVERSITY_BONUS:    80,
    CREW_SPEED_BONUS:           200,    // ⚡ مكافأة ضخمة للسرعة في الكرو
...
var PROXY_REGISTRY = {
    ...
    // ── Core ultra-low-latency proxies (⚡ عمود الكرو الفقري) ───────────
...
// ⚡ CREW BLITZ POOL — فقط الأسرع، فقط ultraBurst، فقط crewCore
//    C01 (4ms·ORANGE·ultraBurst) → C02 (5ms·ZAIN·ultraBurst) → C03 (5ms·UMNIAH) → S01 (24ms·ORANGE·ultraBurst)
...
//
//  ⚡ تغيير جوهري: "crew" و "recruit" أُخرجا من SOCIAL_FORCE
//     وأُضيفا إلى CREW_FORCE — مود جديد مخصص للتجنيد السريع.
//
//  ترتيب الفحص في classifyTraffic مهم:
//    CREW_FORCE    ← يُفحص أولاً (hasOwnProperty ترتيب طبيعي)
//    LOBBY_FORCE   ← يُفحص ثانياً
//    SOCIAL_FORCE  ← يُفحص ثالثاً (بدون crew/recruit)
//    AUTH           ← يُفحص رابعاً
//    GAME_CRITICAL  ← افتراضي
// ────────────────────────────────────────────────────────────────────────────
...
var TRAFFIC_MODES = {
    ...
    // ⚡ CREW_FORCE — مود جديد: تجنيد فائق السرعة
    //    كلمات مفتاحية: crew, recruit, invite, join, apply
    CREW_FORCE: {
        sig:        ["crew","recruit","invite","apply","joinsquad","squadjoin"],
        priority:   11,          // أعلى أولوية
        targetPing: 2,           // مثالي
        maxPing:    CONFIG.CREW_MAX_PING,   // 30ms — يمنع البطيء
        strategy:   "CREW_BLITZ",
        sticky:     true
    },
    ...
    // ⚡ SOCIAL_FORCE — للبحث والاكتشاف فقط (بدون crew/recruit)
    SOCIAL_FORCE: {
        sig:        ["friend","clan","playersearch","social","nearby",
                     "discovery"],
        priority:   10,
        targetPing: 2,
        maxPing:    6,
...
/**
 * ⚡ classifyTraffic: ترتيب الفحص حسّاس.
 *     نفحص CREW_FORCE أولاً لأنه موجود أولاً في TRAFFIC_MODES
 *     (خاصية hasOwnProperty + for...in loop في معظم محركات JS
 *     ترجع الخصائص بترتيب الإضافة).  للتأكيد، نستخدم فحصاً صريحاً:
 *     نفحص CREW أولاً، ثم LOBBY، ثم SOCIAL، ثم AUTH.
 */
function classifyTraffic(host) {
    // ⚡ فحص صريح للكرو أولاً — ضمان المسلك السريع
    var crewSigs = TRAFFIC_MODES.CREW_FORCE.sig;
    for (var c = 0; c < crewSigs.length; c = c + 1) {
    ...
    for (modeKey in TRAFFIC_MODES) {
        ...
        if (modeKey === "CREW_FORCE") continue;  // فحصناه فوق
...
//
//  ⚡ CREW_FORCE يستخدم scoring مختلف:
//     • CREW_SPEED_BONUS (200) للبروكسيات منخفضة الـ ping جداً
//     • crewCore تحصل على مضاعف إضافي
//     • ultraBurst إجباري (إذا CREW_ULTRA_BURST_ONLY مفعّل)
//     • لا يوجد carrier diversity bonus للكرو — السرعة أولاً
// ────────────────────────────────────────────────────────────────────────────
...
var SCORE_CONSTANTS = {
    ...
    CARRIER_DIVERSITY_BONUS: 80,
    CREW_SPEED_BONUS:        200,   // ⚡ مكافأة الكرو فائق السرعة
    CREW_CORE_BONUS:         150    // ⚡ مكافأة crewCore (AMMAN_CORE)
};

/**
 * ⚡ scoreProxy: نسخة محسّنة تدعم CREW_FORCE بمعايير مختلفة.
 */
function scoreProxy(proxy, modeKey, usedCarriers) {
    ...
    // ⚡ CREW bonuses — السرعة فوق كل اعتبار
    if (modeKey === "CREW_FORCE") {
        // كل ما قلّ ping، زادت المكافأة
        score = score + (SCORE_CONSTANTS.MAX_SCORED_PING - proxy.targetPing) * 8;  // ping bonus مضاعف

        // crewCore = AMMAN_CORE = أقل ping في الأردن
        if (proxy.crewCore) score = score + SCORE_CONSTANTS.CREW_CORE_BONUS;

        // مكافأة السرعة الإجمالية
        score = score + SCORE_CONSTANTS.CREW_SPEED_BONUS;
    }

    // Carrier diversity (للسوشيال فقط)
...
/**
 * ⚡ rankProxies: يدعم CREW_FORCE مع فلترة صارمة.
 */
function rankProxies(poolKeys, modeKey) {
    ...
    for (i = 0; i < poolKeys.length; i = i + 1) {
        ...
        // ⚡ CREW BLITZ: فلترة صارمة
        if (isCrewBlitz) {
            if (px.targetPing > CONFIG.CREW_MAX_PING) continue;         // بطيء → مرفوض
            if (CONFIG.CREW_ULTRA_BURST_ONLY && !px.ultraBurst) continue; // بدون ultraBurst → مرفوض
        }
    ...
    // Carrier diversity لSOCIAL فقط — الكرو بدون تنوع
...
function poolForMode(modeKey) {
    // ⚡ CREW → POOL_CREW (أسرع 4 بروكسيات فقط)
