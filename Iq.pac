...
// ═══════════════════════════════════════════════════════════════════════
//  MEGA JORDAN PROXY POOL — 50+ Proxies
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
//  REGIONAL HOP CHAINS — 27 Chains, 4 Hops Each
// ═══════════════════════════════════════════════════════════════════════
...
var SESSION = {
    ...
    isWarm: function() {
        return this.pubgRequests >= 3 && this.age() > 5000;
    },
...
var DNS_STATS = {
    ...
    socialHits: 0, socialAvgTime: 0, regionalHits: 0
};
...
function fastDNS(host) {
    ...
    if (isSocial) {
        ...
        DNS_STATS.socialAvgTime = Math.round(
            (DNS_STATS.socialAvgTime * DNS_STATS.socialHits + dt) / sc
        );
    }
...
var PING = {
    ...
    record: function(ms, mode, host, region) {
        ...
        ms2.count++;
        ms2.total += estimated;
        ms2.min = Math.min(ms2.min, estimated);
        ms2.max = Math.max(ms2.max, estimated);
        ms2.avg = Math.round(ms2.total / ms2.count);
        if (ms2.recent.length >= 5) ms2.recent.shift();
        ms2.recent.push(estimated);

        if (region) {
            if (!this.regionStats[region]) {
                this.regionStats[region] = { count: 0, total: 0, avg: 0 };
            }
            var rg = this.regionStats[region];
            rg.count++;
            rg.total += estimated;
    ...
    regionalAvg: function(r) {
        if (!r || !this.regionalStats[r]) return 999;
        return this.regionalStats[r].avg;
    },
    best: function() {
        if (this.history.length === 0) return 999;
        var b = 999;
    ...
    quality: function(mode) {
        ...
        if (c <= t) return "EXCELLENT";
        if (c <= t * 1.2) return "VERY_GOOD";
        if (c <= mx) return "GOOD";
        if (c <= mx * 1.3) return "ACCEPTABLE";
        return "POOR";
    },
    ...
    variance: function() {
        ...
        var a = this.avg(), ss = 0;
        var s = Math.max(0, this.history.length - 10), c = 0;
        for (var i = s; i < this.history.length; i++) {
            var d = this.history[i].estimated - a;
            ss += d * d;
            c++;
        }
...
// ═══════════════════════════════════════════════════════════════════════
//  MACHINE LEARNING — Triple Layer (Social + Regional + Temporal)
// ═══════════════════════════════════════════════════════════════════════
...
var ML = {
    ...
    recordSuccess: function(mode, route, ping, region, isSocial) {
        ...
        if (!p.routes[route]) {
            p.routes[route] = { uses: 0, totalPing: 0, avgPing: 0 };
        }
        var rd = p.routes[route];
        rd.uses++;
        rd.totalPing += ping;
        rd.avgPing = Math.round(rd.totalPing / rd.uses);
        p.totalSamples++;
        if (rd.avgPing < p.bestPing && rd.uses >= 3) {
            p.bestPing = rd.avgPing;
            p.bestRoute = route;
        }
        ...
        if (CFG.TEMPORAL_ML) {
            ...
            if (!this.temporalPatterns[tKey]) {
                this.temporalPatterns[tKey] = { bestRoute: null, bestPing: 999, samples: 0 };
            }
            this.temporalPatterns[tKey].samples++;
            if (ping < this.temporalPatterns[tKey].bestPing) {
                this.temporalPatterns[tKey].bestPing = ping;
                this.temporalPatterns[tKey].bestRoute = route;
            }
        }
    ...
    predict: function(mode, region) {
        ...
        var key = mode + "_" + region;
        var pat = this.patterns[key];
        if (!pat || pat.totalSamples < 5) return null;
        return pat.bestRoute;
    },
...
function calcHealthScore(h, p) {
    ...
    s -= (100 - h.uptime) * 0.4;
    if (h.load > 90) s -= 20;
    else if (h.load > 75) s -= 10;
    else if (h.load > 60) s -= 5;
    if (p && h.avgPing) {
...
function getBestProxies(tier, carrier, count, socialOptimized, regionHop) {
    ...
    for (var name in PROXY) {
        ...
        candidates.push({ name: name, score: calcProxyScore(p, h, socialOptimized) });
    }
...
function getRegionalProxies(region, count, socialOptimized) {
    ...
    if (results.length === 0) {
        ...
        var mapped = region;
        if (region && region.indexOf("AMMAN") !== -1) mapped = "AMMAN_CORE";
        else if (regionMap[region]) mapped = regionMap[region];
        results = getBestProxies(undefined, undefined, count, socialOptimized, mapped);
    }
...
function calcProxyScore(p, h, social) {
    ...
    if (h.load <= 50) s += 10;
    else if (h.load <= 70) s += 7;
    else if (h.load <= 85) s += 4;
    if (social && p.socialOptimized) {
        s += 20;
        if (p.socialDedicated) s += 15;
    }
    return Math.round(s);
}
...
var GUARD = {
    ...
    checkDestination: function(ip, host, mode) {
        ...
        if (ip.indexOf(":") !== -1) {
            if (CFG.JORDAN_ONLY_MODE || CFG.BLOCK_INTERNATIONAL) {
                this.blockedHosts[host] = { reason: "IPv6", time: now() };
                SESSION.blockedHits++;
                return false;
            }
        }
        ...
        if (m && m.socialPriority && CFG.FORCE_JORDAN_SOCIAL) {
            this.blockedHosts[host] = { reason: "Social requires JO", time: now() };
            SESSION.blockedHits++;
            return false;
        }
        if (m && m.priority >= 9 && (CFG.FORCE_JORDAN_LOBBY || CFG.FORCE_JORDAN_MATCHMAKING)) {
            this.blockedHosts[host] = { reason: "Critical requires JO", time: now() };
            SESSION.blockedHits++;
            return false;
        }
        if (CFG.JORDAN_ONLY_MODE) {
            this.blockedHosts[host] = { reason: "JO only", time: now() };
            SESSION.blockedHits++;
            return false;
        }
        if (CFG.BLOCK_INTERNATIONAL) {
            this.blockedHosts[host] = { reason: "Intl blocked", time: now() };
            SESSION.blockedHits++;
            return false;
        }
        return true;
    ...
    buildChain: function(names, mode, req) {
        ...
        for (var i = 0; i < names.length; i++) {
            ...
            var isRegional = !!proxy.regionHop;
...
function stickySet(key, value, ttl) {
    STICKY[key] = {
        value: value, created: now(), ttl: ttl || CFG.STICKY_TTL,
        hits: 0, lastHit: now()
    };
}
...
function detectRegion(host, ip) {
    ...
    var jordanPatterns = [
        ...
        "madaba", "tafilah", "maan", "petra", "me-jo", "mena-jo"
    ];
...
function assessServerQuality(ip, host, mode) {
    ...
    var m = MODES[mode];
    if (m && m.socialPriority && ip && !GUARD.isJordan(ip)) score -= 60;
...
var CONNECTION = {
    profile: function() {
        ...
        if (avg <= 4 && best <= 2) { type = "5G_ULTRA"; quality = "EXCELLENT"; }
        else if (avg <= 6 && best <= 4) { type = "5G_PREMIUM"; quality = "EXCELLENT"; }
        else if (avg <= 10) { type = "5G_GOOD"; quality = "VERY_GOOD"; }
        else if (avg <= 18) { type = "4G_EXCELLENT"; quality = "GOOD"; }
        else if (avg <= 30) { type = "4G_GOOD"; quality = "FAIR"; }
        else { type = "WEAK"; quality = "POOR"; }
        SESSION.networkQuality = quality;
        return {
            type: type, quality: quality, avgPing: avg,
            bestPing: best, tier: this.getTier(type)
        };
    },
    ...
    boost: function() {
        var p = this.profile();
        if (p.type === "5G_ULTRA") return 90;
        if (p.type === "5G_PREMIUM") return 80;
        if (p.type === "5G_GOOD") return 60;
        if (p.type === "4G_EXCELLENT") return 45;
        if (p.type === "4G_GOOD") return 25;
        return -30;
    }
};
...
var TIME = {
    ...
    getBoost: function() {
        ...
        if (peak && weekend) return 35;
        if (peak || weekend) return 20;
        return 0;
    }
};
...
function getCarrierBonus(carrier) {
    ...
    if (carrier === "ZAIN") return 47;
    if (carrier === "UMNIAH") return 42;
    if (carrier === "JT") return 32;
    return 0;
}
...
function calculateScore(ip, host, port, dns, mode) {
    ...
    // FACTOR 1: Mode Priority
    if (m) {
        score += m.priority * 5;
        if (m.socialPriority) score += 40;
        if (m.matchmakingSpeed) score += m.matchmakingSpeed * 6;
    } else {
        score += 25;
    }

    // FACTOR 2: DNS Performance
    var dt = dns.dt;
    if (dt <= 1) score += 55;
    else if (dt <= 3) score += 50;
    else if (dt <= 6) score += 42;
    else if (dt <= 12) score += 32;
    else if (dt <= 25) score += 20;
    else score -= 30;

    // FACTOR 3: Ping Health
    ...
    else if (quality === "ACCEPTABLE") score += 8;
    else score -= 40;

    // FACTOR 4: JORDAN IP MEGA BONUS
    if (ip && GUARD.isJordan(ip)) {
        score += 200;
        ...
        var city = GUARD.getJordanCity(ip);
        var cityBonus = 0;
        if (city === "AMMAN_CORE") cityBonus = 45;
        else if (city === "AMMAN_METRO") cityBonus = 40;
        else if (city === "AMMAN_CITY") cityBonus = 35;
        else if (city === "AMMAN_NORTH" || city === "AMMAN_SOUTH") cityBonus = 32;
        else if (city === "SWEILEH" || city === "NAUR" || city === "JUBEIHA" ||
                 city === "WADI_AL_SEER" || city === "MARKA" || city === "QUEISMEH") cityBonus = 33;
        else if (city === "IRBID" || city === "IRBID_CITY") cityBonus = 28;
        else if (city === "ZARQA" || city === "ZARQA_CITY") cityBonus = 28;
        else if (city === "SAHAB") cityBonus = 30;
        else if (city === "AQABA" || city === "AQABA_CITY") cityBonus = 25;
        else if (city === "BALQA" || city === "SALT") cityBonus = 23;
        else if (city === "MADABA") cityBonus = 23;
        else if (city === "JERASH" || city === "AJLOUN") cityBonus = 20;
        else if (city === "KARAK") cityBonus = 18;
        else if (city === "MAFRAQ" || city === "RAMTHA") cityBonus = 18;
        else if (city === "TAFILAH") cityBonus = 16;
        else if (city === "MAAN" || city === "PETRA") cityBonus = 16;
        else if (city === "FUHEIS" || city === "MAHIS" || city === "AIN_BASHA") cityBonus = 22;
        else score += 12;
        score += cityBonus;
        SESSION.recordRegionalHit(city);
    ...
    // FACTOR 5: Host Pattern
    var region = detectRegion(host, ip);
    if (region.region === "JORDAN") {
        score += 90 + (region.confidence * 0.2);
    } else {
        score -= 80;
    }

    // FACTOR 6: Connection Quality
    ...
    // FACTOR 12: Time Optimization
    score += TIME.getBoost();

    // FACTOR 13: Server Quality
    ...
    // FACTOR 15: Social Endpoint Bonus
    if (dns.socialEndpoint && CFG.ENABLE_FRIEND_DISCOVERY) {
        score += 70;
        if (ip && GUARD.isJordan(ip)) score += 40;
    }

    // FACTOR 16: Regional Endpoint Bonus
    if (dns.regionalEndpoint && CFG.ENABLE_PROVINCE_ROUTING) {
        score += 45;
        if (ip && GUARD.isJordan(ip)) score += 28;
    }

    // FACTOR 17: Visibility Boost Multiplier
    if (m && m.visibilityBoost) {
        score = Math.round(score * (1 + (m.visibilityBoost * 0.1)));
    }

    // FACTOR 18: Matchmaking Speed Boost
    if (m && m.matchmakingSpeed) {
        score = Math.round(score * (1 + ((m.matchmakingSpeed - 1) * 0.15)));
    }

    // FACTOR 19: Peak Hour Social Bonus
    if (m && m.socialPriority && TIME.isPeakHours()) {
        score += 30;
    }

    // FACTOR 20: Weekend Boost
    if (TIME.isWeekend() && m && m.socialPriority) {
        score += 20;
    }

    // FACTOR 21: Carrier Match Bonus
    if (ip && GUARD.isJordan(ip)) {
        var c = getCarrier(ip);
        if (c === "ORANGE") score += 10;
    }

    // FACTOR 22: Deep Scan Bonus
    if (CFG.ENABLE_DEEP_SCAN && m && m.searchRadius && m.searchRadius >= 300) {
        score += 15;
    }
...
function selectRoute(mode, score, ip, port, host, dns) {
    ...
    if (PING.isCritical()) {
        ...
        var emergency = GUARD.buildChain(
            getBestProxies(0, null, 4, m.socialPriority), mode,
    ...
    if (strategy === "SOCIAL_ULTRA_FORCE") {
        if (city && city !== "UNKNOWN" && CFG.REGIONAL_HOP_CHAINS) {
            ...
            if (socialHops) {
                route = GUARD.buildChain(socialHops, mode,
        ...
        if (!route) {
            if (score >= 90 || (ip && GUARD.isJordan(ip))) {
                route = GUARD.buildChain(
                    ["SOCIAL_ORANGE_1", "SOCIAL_ZAIN_1", "SOCIAL_JT_1", "SOCIAL_UMNIAH_1"], mode,
                    { burst: true, ultraBurst: true, hyperBurst: true, tier: 0, socialOptimized: true });
            } else if (score >= 75) {
                route = GUARD.buildChain(
                    getBestProxies(0, null, 4, true), mode,
                    { burst: true, ultraBurst: true, tier: 0, socialOptimized: true });
            } else {
                route = GUARD.buildChain(
                    ["SOCIAL_ORANGE_1", "ORANGE_ULTRA_1", "ZAIN_ULTRA_1", "JT_ULTRA_1"], mode,
                    { burst: true, ultraBurst: true, hyperBurst: true, tier: 0, socialOptimized: true });
            }
        }
    }
    // ━━━ LOBBY_ULTRA_FORCE ━━━
        ...
        if (!route) {
            if (score >= 85 || (ip && GUARD.isJordan(ip))) {
                var proxyList;
                if (carrier === "ORANGE" || score >= 92) {
                    proxyList = getBestProxies(0, "ORANGE", 4, true);
                } else {
                    proxyList = getBestProxies(0, carrier, 4, true);
                }
                route = GUARD.buildChain(proxyList, mode,
                    { burst: true, ultraBurst: true, hyperBurst: true, tier: 0, socialOptimized: true });
            } else {
                route = GUARD.buildChain(
                    ["ORANGE_ULTRA_1", "ZAIN_ULTRA_1", "JT_ULTRA_1", "UMNIAH_ULTRA_1"], mode,
                    { burst: true, ultraBurst: true, tier: 0, socialOptimized: true });
            }
        }
    }
    ...
    // ━━━ SOCIAL_GAME_CRITICAL ━━━
        ...
        if (score >= 80 || (ip && GUARD.isJordan(ip))) {
            route = GUARD.buildChain(
                getBestProxies(0, carrier, 4, true), mode,
                { burst: true, tier: 0, socialOptimized: true });
        } else {
            route = GUARD.buildChain(
                getBestProxies(0, null, 4, false), mode,
                { burst: true, tier: 0 });
        }
    }
    // ━━━ GAME_ULTRA_CRITICAL ━━━
        ...
        if (!route) {
            if (score >= 85 || (ip && GUARD.isJordan(ip))) {
                route = GUARD.buildChain(
                    getBestProxies(0, carrier, 4, false), mode,
                    { burst: true, ultraBurst: true, hyperBurst: true, tier: 0 });
            } else {
                route = GUARD.buildChain(
                    getBestProxies(0, null, 4, false), mode,
    ...
    // ━━━ SECURE_CRITICAL ━━━
        ...
        route = GUARD.buildChain(
            ["ORANGE_ULTRA_1", "SOCIAL_ORANGE_1", "ZAIN_ULTRA_1", "JT_ULTRA_1"], mode, requirements);
    }
    // ━━━ GAME_CRITICAL ━━━
    else if (strategy === "GAME_CRITICAL") {
        if (score >= 80 || (ip && GUARD.isJordan(ip))) {
            route = GUARD.buildChain(
                getBestProxies(0, carrier, 4, false), mode, requirements);
        } else {
            route = GUARD.buildChain(
                getBestProxies(1, null, 4, false), mode, requirements);
        }
    }
    // ━━━ OTHER STRATEGIES ━━━
    else {
        route = GUARD.buildChain(
            getBestProxies(0, null, 4, false), mode, requirements);
    }
...
function generatePerformanceReport() {
    ...
    return {
        ...
        ml: {
            ...
            social: Object.keys(ML.socialPatterns).length,
            regional: Object.keys(ML.regionalPatterns).length,
            temporal: Object.keys(ML.temporalPatterns).length
        },
...
//  🏘️ 27 REGIONAL HOP CHAINS x 4 HOPS EACH
//  🔄 50+ PROXIES ACROSS ALL GOVERNORATES
//  🧠 TRIPLE ML: Social + Regional + Temporal
