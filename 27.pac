...
//  PUBG ID: 5744469523
//  Version: 2026.7 - RANGE SWITCHES (JO / KW keys)
//
//  THE iOS TRUTH (honest):
//    * iOS PAC routes TCP only. PUBG in-match traffic is UDP -> use a tunnel
//      app (Shadowrocket / Stash / Quantumult X) with UDP relay for that.
//    * What this PAC controls on iPad = PUBG's TCP parts (login, API, store)
//      + how Jordanian & Kuwaiti addresses are taken from the ranges.
//
//  PROXY (verified): 94.127.211.6:10010 - Amman, Jordan, AS9038 Batelco/Orbit
// ============================================================================

// ============================================================================
//  CONFIG - the switches (edit ONLY here)
// ============================================================================

// [1] Where PUBG's TCP parts go (login / API / store / lobby HTTP)
var TCP_ROUTE = "PROXY 94.127.211.6:10010";   // Jordan exit IP

// [2] Everything else (browsing, other apps)
var DEFAULT_ROUTE = "DIRECT";

// [3] SWITCH_JO - the Jordan ranges key
//     ON  = take Jordanian addresses FROM the JO ranges and route them
//           DIRECT (lowest local ping, no proxy hop)
//     OFF = skip the JO ranges (they fall through to DEFAULT_ROUTE)
var SWITCH_JO = "ON";

// [4] SWITCH_KW - the Kuwait ranges key (same behavior)
var SWITCH_KW = "ON";
// ============================================================================
...
function FindProxyForURL(url, host) {
    ...
    // BLOCK 2 - PUBG Mobile TCP parts -> TCP_ROUTE (Jordan proxy)
    ...
    // BLOCK 3 - Jordan + Kuwait official RIPE ranges
    //   Controlled by SWITCH_JO / SWITCH_KW (the keys above).
    if (isIpLiteral(host)) {
        if (SWITCH_JO === "ON") {
            // Jordan (JO) - official RIPE ranges -> taken DIRECT
        ...
        if (SWITCH_KW === "ON") {
            // Kuwait (KW) - official RIPE ranges -> taken DIRECT
